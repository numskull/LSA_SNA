...
# load("~/Desktop/Parsing the Imprint/tcp_dates.rda")
# load("~/Desktop/Parsing the Imprint/tcp_imps.rda")
...
### ===== How TCP dates were created, NAs removed ===== ####
# Prep dates vector for dates of each TCP file
dates = as.character(tcp$Date[which(tcp$Status == "Free")])
dates = as.integer(dates)
# Will generate some NA values. With date range, just default to early date
nullDates = tcp$Date[which(is.na(dates) == T)]
...
for (i in range) {
  ...
  for (j in 1:length(peeps)) {
    if (j > 0) {
      row = which(rownames(bbti) == peeps[j])
      namej = paste(bbti[row,], collapse="; ")
    }
    names = c(names,namej)
